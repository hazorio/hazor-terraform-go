...
func (d *ImageDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Look up a Hazor image by ID.",
		Attributes: map[string]schema.Attribute{
			"id":     schema.StringAttribute{Description: "Image ID.", Required: true},
			"name":   schema.StringAttribute{Description: "Image name.", Computed: true},
...
func (d *ImageDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	result, err := d.client.Read(ctx, fmt.Sprintf("/api/v1/images/%s", state.ID.ValueString()))
