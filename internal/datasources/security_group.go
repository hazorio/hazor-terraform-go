...
func (d *SecurityGroupDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Look up a Hazor security group by ID.",
		Attributes: map[string]schema.Attribute{
			"id":     schema.StringAttribute{Description: "SecurityGroup ID.", Required: true},
			"name":   schema.StringAttribute{Description: "SecurityGroup name.", Computed: true},
			"status": schema.StringAttribute{Description: "SecurityGroup status.", Computed: true},
		},
	}
}
...
func (d *SecurityGroupDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	result, err := d.client.Read(ctx, fmt.Sprintf("/api/v1/security-groups/%s", state.ID.ValueString()))
	if err != nil { resp.Diagnostics.AddError("Error reading security group", err.Error()); return }
